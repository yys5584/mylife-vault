...
인터뷰가 끝나면 셋업 완료 보고서가 나오고, 보고서 끝에 "이제 이렇게 쓰시면 됩니다" 안내가 붙는다. 쓰는 방법은 두 가지다.

### 고민 상담 — 상황을 그대로 던진다

결론 한 줄, 근거, 지금 할 것, 가드 순서로 답한다.

| 고민 | LLM에게 | 근거 문서 |
|---|---|---|
| 이직·커리어 | "이직 제안이 왔어. 연봉은 20% 오르는데 초기 스타트업이야. 갈까?" | `career_style.md` + `philosophy.md` + `self_profile.md` |
| 지금 하는 일 | "요즘 일이 손에 안 잡혀. 번아웃인지 봐줘" | `career_style.md` + `self_profile.md` + `roadmap.md` |
| 연애 | "만난 지 한 달 됐는데 서운한 게 쌓였어. 지금 말해도 될까?" | `love_style.md` + `relationship_protocol.md` + `self_profile.md` |
| 재테크 점검 | "이번 달 매매 기록이야. 내 룰 어긴 거 있나 봐줘" | `investment_style.md` + `philosophy.md` |
| 큰 결정 | "퇴사할지 고민이야. 의사결정 필터 돌려줘" | `life_compass.md` + `philosophy.md` |
| 정기 점검 | "이번 주 돌아봐줘" | `roadmap.md` + `life_compass.md` |

재테크는 종목·상품을 추천하지 않는다. 내가 정한 룰 안인지 밖인지만 점검한다.

### 깊이 풀이 — 영역별로 나를 푼다

사주·별자리는 명시 요청 시에만 같이 얹는다.
