...
> CLI 초보자용. 어차피 Claude Code 있으면 *Claude한테 다 시키면 됨*.
> Python 설치·git·pip 같은 거 손으로 칠 필요 없음.
...
### 1. Claude Code 설치 (1회)
https://claude.com/claude-code → 본인 OS 다운로드 → 설치 → `claude` 명령으로 진입.
...
내 홈 폴더에 클론 → 의존성 설치 → 사주·별자리 자동 계산 → /mylife-setup 인터뷰 시작.
Python·pip 명령은 알아서. 막히면 물어봐.
```

→ Claude가 알아서:
1. `git clone` → 본인 컴퓨터에 다운로드
2. `pip install -r scripts/requirements.txt` (권한 문제 만나면 `--user` 자동 추가)
3. 본인 정보 받아서 `python scripts/calc_saju.py` + `calc_zodiac.py` 실행
4. 결과 검증 후 인터뷰 시작

**Python 없으면**: Claude가 "Python 먼저 설치해야 함" 안내 + 본인 OS에 맞는 명령 알려줌. 따라하면 됨.
...
중간에 멈추기: "여기까지 저장하고 다음에 계속" 말하면 Claude가 저장 + 종료.
...
vault 채워지면 어떤 LLM이든 이 폴더 컨텍스트로 받으면 본인 맞춤 답변.
...
**Q. 출생 시간 모름**
- "모름"으로 진행. 사주는 시주만 빠지고 나머지 정확. 별자리는 태양궁만 정확 (달궁·상승궁 의미 없음).
...
본인 vault 백업 원하면 → Claude한테 *"이 vault 내 private GitHub repo로 push해줘"* 시키기. (private repo 이름만 정해주면 알아서.)
...
> CLI는 *Claude한테 시키는 인터페이스*. 본인이 손으로 칠 필요 없음.
